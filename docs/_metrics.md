...
| 测试数 (pytest) | **816** | `pytest --collect-only` |
| 测试数 (static) | 785 | grep `def test_*` in tests/ |
...
| 测试数演进 (ROADMAP) | [546, 560, 562, 586, 642, 700, 802, 816] | grep `Final test count: N/N passing` |
| Recorded commit SHA | `d291b25` | recent reachable commit; `--check` allows follow-up metrics commits |
...
- ✅ 版本一致：两份都是 `v2.3.1`。
- ✅ 步骤数一致：14。
- ℹ️ pytest (816) vs 静态 (785) 差距 >20，多出来的来自 parametrize/fixture——以 pytest 为准。
