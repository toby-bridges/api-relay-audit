...
| 测试数 (pytest) | **776** | `pytest --collect-only` |
| 测试数 (static) | 752 | grep `def test_*` in tests/ |
...
- ✅ 版本一致：两份都是 `v2.3`。
- ✅ 步骤数一致：14。
- ℹ️ pytest (776) vs 静态 (752) 差距 >20，多出来的来自 parametrize/fixture——以 pytest 为准。
- ⚠️ ROADMAP 最新记录 769 个测试，但当前 pytest 是 776。要么 ROADMAP 漏更新，要么有未记录的新测试。
