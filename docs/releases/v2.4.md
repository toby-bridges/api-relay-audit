...
The root `SKILL.md` is the ClawHub bundle. Registry publication is a separate
external write and is not claimed by these release notes; fresh
inspect/install/check results are recorded only after that write succeeds.

Hermes uses the repository tap as its supported entrypoint:
