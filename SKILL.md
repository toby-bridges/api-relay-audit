...
| 10 | Stream integrity (流完整性, AC-1 SSE-level) | Opens an Anthropic streaming request with thinking enabled and verifies known event types, monotonic `output_tokens`, consistent `input_tokens`, non-empty thinking signatures, exactly one terminal `message_stop` after `message_start` (trailing `ping` allowed), and a Claude stream model identity. The existing signals are sourced from hvoy.ai `claude_detector.py`; terminal completeness is a local protocol hardening. |
...
```bash
python3 -c "import urllib.request; urllib.request.urlretrieve('https://raw.githubusercontent.com/toby-bridges/api-relay-audit/v2.3.1/audit.py', 'audit.py')"
```
