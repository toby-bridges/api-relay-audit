...
- `api_relay_audit/infra_fingerprint.py` — Relay-framework fingerprinting (Step 12, v1.8, informational). Fires 3 unauthenticated GET probes (`/`, `/v1/models`, `/nonexistent-abc12345xyz`) and matches response headers + body against a signature database covering new-api, one-api, lobechat-relay, fastgpt, cloudflare, nginx-raw, caddy-raw. `classify_framework(headers, body)` returns `(framework, matched_signals)` with declaration-order precedence (specific → generic). `aggregate_framework(results)` runs majority vote across 3 probes: ≥2 hits → `confirmed`, 1 hit → `tentative`, 0 → `unknown`. Body scan capped at 8 KB. Does NOT feed into the 6D risk matrix — purely informational; rationale sourced from Zhang et al., *Real Money, Fake Models*, arXiv:2603.01919, Section 3.2 Infrastructure.
- `api_relay_audit/latency_variance.py` — Latency-variance fingerprinting (Step 13, v1.8, informational). Fires N (default 10, range 3-50 via `validate_probe_count`) identical minimal requests (`max_tokens=8`) and measures per-request end-to-end latency with `time.perf_counter` (v1.8.1: was `time.time`, swapped for monotonicity). `APIClient.ensure_format()` is called before the timing loop so the first sample does not silently include a failed Anthropic probe + successful OpenAI request on OpenAI-compatible relays. `summarize_latencies` returns min/median/max/mean/stdev/cv; `detect_bimodality` flags distributions where largest-inter-sample-gap / median > 0.5 *and* both clusters have ≥2 samples; `classify_variance` returns `stable` (CV<0.25) / `variable` (0.25≤CV<0.5) / `high-variance` (CV≥0.5) / `bimodal` / `inconclusive` (<3 successful samples). Does NOT feed into the 6D risk matrix — informational only. Bimodality is the strong signal for silent A/B testing between the advertised model and a cheaper substitute.
- `scripts/audit.py` — 13-step audit orchestration: Infrastructure → Models → Token Injection → Prompt Extraction → Instruction Conflict + Identity → Jailbreak → Context Length → Tool-Call Substitution → Error Response Leakage → Stream Integrity → Web3 Injection (profile-gated) → Infra Fingerprint → Latency Variance. Steps 12/13 are informational only and do not feed the 6D risk matrix. Step 4/6 share a broadened `REFUSAL_MARKERS` vocabulary + `_looks_like_refusal` helper so benign refusals do not trip the identity-keyword leak detector (v1.7.4). Overall rating uses a **6D risk matrix**: D1 = injection > 100, D2 = instruction overridden, D3 = any tool-call substitution, D4 = error response critical/high leakage, D5 = stream integrity anomaly, D6 = Web3 prompt injection (only active under `--profile web3|full`). HIGH if D3 or D4 or D5 or D6 OR (D1 AND D2); MEDIUM if D1, D2, D3i, D4i (inconclusive), D4m (medium-only), D5i (stream inconclusive), or D6i (web3 inconclusive); LOW otherwise. Plus the `--profile` selector (general / web3 / full) gates which step set runs — rejected git branch forking in favor of a runtime flag so the dual-distribution invariant stays intact.
